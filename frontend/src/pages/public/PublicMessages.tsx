...
export default function PublicMessages() {
  ...
  // 0. Safety Check (Fixes "user possibly null" error)
  if (!user) return null;
           ...
           <div className="p-4 border-b border-gray-200 bg-white">
               ...
               <div className="relative">
                   ...
                   <input 
                     ...
                     className="w-full pl-9 p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-judicial-gold outline-none transition-all"
                   />
               </div>
           </div>
           ...
           <div className="overflow-y-auto flex-1">
              ...
              {!loading && inbox.length === 0 && (
                  <div className="p-8 text-center text-gray-400 flex flex-col items-center">
                      ...
                      <button onClick={() => navigate('/public/connect')} className="text-xs text-judicial-gold font-bold mt-2 hover:underline">Find a Lawyer</button>
                  </div>
              )}
              ...
              {inbox.map((contact) => (
                  <div 
                    ...
                    className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-white transition-all flex gap-3 items-center
                        ${activeChat?.id === contact.id ? 'bg-white border-l-4 border-l-judicial-gold shadow-sm' : 'border-l-4 border-l-transparent'}
                    `}
                  >
                      {/* Avatar */}
                      <div className="w-10 h-10 rounded-full bg-judicial-black text-judicial-gold flex items-center justify-center font-bold text-sm shrink-0">
                          {contact.name[0]}
                      </div>
        ...
        {activeChat ? (
            <div className="flex-1 flex flex-col bg-[#F3F4F6]">
                ...
                <div className="p-4 border-b border-gray-200 bg-white shadow-sm flex justify-between items-center z-10">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-judicial-gold text-black rounded-full flex items-center justify-center font-bold">
                            {activeChat.name[0]}
                        </div>
                        <div>
                        ...
                        return (
                            <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                <div className={`
                                    ...
                                    ${isMe 
                                        ? 'bg-judicial-black text-white rounded-tr-none' 
                                ...
                                `}>
                                    ...
                                    <div className={`text-[10px] mt-1 flex items-center justify-end gap-1 ${isMe ? 'text-gray-400' : 'text-gray-400'}`}>
                                        ...
                                        {isMe && <CheckCheck size={12} className="text-judicial-gold"/>}
                                    </div>
                                </div>
                            </div>
                        )
                ...
                <div className="p-4 bg-white border-t border-gray-200">
                    <div className="flex items-end gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200 focus-within:border-judicial-gold focus-within:ring-1 focus-within:ring-judicial-gold/20 transition-all">
                        ...
                        <button 
                            onClick={sendMessage}
                            disabled={!input.trim()} 
                            className="p-2 bg-judicial-gold text-black rounded-lg hover:bg-judicial-gold-dark disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm"
                        >
                            <Send size={18} />
                        </button>
                    </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-gray-50/50">
                <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-sm mb-6">
                    <MessageSquare size={40} className="text-judicial-gold" />
                </div>
                ...
                <button 
                    ...
                    className="px-6 py-3 bg-judicial-black text-white font-bold rounded-xl hover:bg-gray-800 transition-all shadow-md flex items-center gap-2"
                >
